...
# 4 : input1_Temp = entry at single timestep of ave daily temperature 
# 5 : input2_DayHrs = entry at single timestep of ave max num of daylight hrs 
# 6 : input3_Wet = entry at single timestep of ave/normalized waterstation level of SB02HB 
###################################################%
...
# step 3: Assuming the input1_Temp is above min temp, kill off Larvae-pupae based on temp and 
...
cpmod = function(time, yinout, parms, input1_Temp, input2_DayHrs, input3_Wet, N = 100){
  ...
  # first I solve the reaction part of the models for each life stage:
    ...
    ddmortLP = calc_mosq_atr(input3_Wet, parms$LP_Mortality)
    #ddmortLP = as.numeric(exp(alpha1 + beta1*input3_Wet)) # density-dependent mortality coefficient
    ...
    #dimortAD = as.numeric(PsiAd*(273.15 + input1_Temp)*exp(-AEAd/(8.3144598*(273.15 + input1_Temp))))
    dimortAD = calc_mosq_atr(input1_Temp, parms$Adult_Mortality)
  ...
  #DiapProp = 1.0/(1.0 + exp(-(DiapInt - DiapCoef1*input2_DayHrs)))
  DiapProp = calc_mosq_atr(input2_DayHrs, parms$Diapause)
  # DiapProp = 0
    
  # setting up maturation velocities
  if(input1_Temp > as.numeric(parms$Cutoff)){
    #vtE = as.numeric(PsiEggs*(273.15 + input1_Temp)*exp(-AEE/(8.3144598*(273.15+input1_Temp)))) # for eggs
    vtE = calc_mosq_atr(input1_Temp, parms$Egg)
  }else{
    vtE = 0.0
  }
  ...
  if(input1_Temp > as.numeric(parms$Cutoff)){
    #vtA = as.numeric(IntAd + Coef1Ad*input1_Temp) # for adults
    vtA = calc_mosq_atr(input1_Temp, parms$GP)
  }else{
    vtA = 0.0
  }
  ...
  if(input1_Temp > as.numeric(parms$Cutoff)){
    ...
    # vtA which had been evaluated based on TempC input1_Temp but after some analysis we see 
