...
daylight.hrs.day <- function(latitude,longitude,date,daylength_def=0.8333){
  if (!is.element("tidyverse", installed.packages()[,1])){ # check if tidyverse is installed
    install.packages("tidyverse",)
  }
  ...
  day_of_year <- as.numeric(format.Date(date,format="%j"))
  theta=0.2163108 +2*atan(0.9671396*tan(0.00860*(day_of_year-186)))
  phi=asin(0.39795*cos(theta))
  DayLength=24-24/pi*acos((sin(daylength_def*pi/180)+
                             sin(latitude*pi/180)*sin(phi))/
                            (cos(latitude*pi/180)*cos(phi)))
...
# daylight.hrs.day(latitude,longitude,start_date)
